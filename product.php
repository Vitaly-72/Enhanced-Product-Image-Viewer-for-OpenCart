		...
		if ($product_info) {
			...
			if ($product_info['image']) {
				...
				if ($this->request->server['HTTPS']) {
					$data['original'] = $this->config->get('config_ssl') . 'image/' . $product_info['image'];
				} else {
					$data['original'] = $this->config->get('config_url') . 'image/' . $product_info['image'];
				}
			} else {
				$data['popup'] = '';
				$data['original'] = '';
			...
			foreach ($results as $result) {
				...
				if ($this->request->server['HTTPS']) {
					$original_url = $this->config->get('config_ssl') . 'image/' . $result['image'];
				} else {
					$original_url = $this->config->get('config_url') . 'image/' . $result['image'];
				}
